...
<div class="row">
    <div class="col-md-6">
        <!-- <a href="{{ Route('Worker.create') }}"><button type="buuton" class="btn btn-primary w-md">Add New
                Record</button></a> -->
                 @if(isset($CheckForm) && $CheckForm->write_access == 1)
    <a href="{{ route('Worker.create') }}" class="btn btn-primary">Add New Record</a>
@else
                ...
                <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $Worker->email_id}}</td>
                            <td>{{ $Worker->pan_no}}</td>
                            <td>{{ $Worker->address}}</td>
                            <td>{{ $Worker->adhar_no}}</td>
                            @if($CheckForm->edit_access==1)
                            <td>
                                <a class="btn btn-outline-secondary btn-sm edit"
                                    href="{{route('Worker.edit', $Worker->employee_id)}}" title="Edit">
                            ...
                            <td>
                                <button class="btn   btn-sm delete" data-placement="top" id="DeleteRecord"
                                    data-token="{{ csrf_token() }}" data-id="{{ $Worker->employee_id }}"
                                    data-route="{{route('Worker.destroy', $Worker->employee_id )}}" title="Delete">
