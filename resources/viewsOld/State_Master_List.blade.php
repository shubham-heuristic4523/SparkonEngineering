      ...
      <div class="row">
          <div class="col-md-6">
              <!-- <a href="{{ Route('State.create') }}"><button type="buuton" class="btn btn-primary w-md">Add New
                      Record</button></a> -->
                       @if(isset($CheckForm) && $CheckForm->write_access == 1)
    <a href="{{ route('State.create') }}" class="btn btn-primary">Add New Record</a>
@else
                              ...
                              <tr>
                                  ...
                                  <td>
                                      <a class="btn btn-outline-secondary btn-sm edit"
                                          href="{{route('State.edit', $row->state_id)}}" title="Edit">
                                  ...
                                  <td>
                                      <a class="btn btn-outline-secondary btn-sm view"
                                          href="{{route('State.show', $row->state_id)}}" title="View">
                                  ...
                                  <td>
                                      <button class="btn btn-outline-secondary btn-sm delete" data-placement="top"
                                          id="DeleteRecord" data-token="{{ csrf_token() }}"
                                          data-id="{{ $row->state_id }}"
                                          data-route="{{route('State.destroy', $row->state_id)}}" title="Delete">
