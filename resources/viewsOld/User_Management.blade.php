    ...
    <td>
        <input type="checkbox" class="checkBox approve_chk" name="chka{{ $no }}" value="{{ $row->form_code }}"
            ...
            @endforeach>
                                ...
                                ->whereIn('employeeCode',explode(",",$row->employeeCode))->get();

                                @endphp
                                <td>
<select name="approve_by{{ $no }}[]" class="approve_by_select" style="width:100px" multiple>

                                        @foreach($emplist as $rowEmp)
                                        <option value="{{ $rowEmp->employeeCode }}" @foreach($formlistbyuser as $rowc)
                                            {{ in_array($rowEmp->employeeCode,explode(",",$rowc->approve_by)) ? 'selected="selected"' : '' }}
                                            @endforeach>{{ $rowEmp->fullName }}</option>
                                        @endforeach
...
<select name="approve_by{{ $no }}[]" class="approve_by_select" style="width:100px" multiple>
                                        ...
                                        @foreach($emplist as $rowEmp)
                                        <option value="{{ $rowEmp->employeeCode }}">{{ $rowEmp->fullName }}</option>
                                        @endforeach
